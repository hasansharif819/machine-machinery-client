...
const Purchase = () => {
    ...
    const { data: p, refetch, isLoading } = useQuery('product', () => fetch(`https://hello-tools-updated-server.vercel.app/api/v1/products/${purchaseID}`).then(res => res.json()))
    if (isLoading) {
        return <Loading />
    }
    ...
    const addToCart = () => {
        if (user) {
            ...
            const url = `https://hello-tools-updated-server.vercel.app/api/v1/cart`;
            fetch(url, {
                method: "POST",
                headers: {
    ...
    const buyNow = event => {
        ...
        if (user) {
            ...
            const url = `https://hello-tools-updated-server.vercel.app/api/v1/order`;
            fetch(url, {
                method: "POST",
                headers: {
                    'content-type': 'application/json'
